...
- [x] **9.0 Tool calling in the live chat path.** Closed 2026-09-19, option
      (b): `kura-llm` gained `ToolSpec`/`ToolCall` (`Message.toolCalls`,
      `Message.toolCallId`, `CreateDispatchInput.tools`,
      `Dispatch.tools`/`toolCalls`, provider request/response), persisted on
      the dispatch row (schema v5: `tools_json`, `tool_calls_json`), and
      `chat::Service` runs a bounded round loop behind a `ToolHost` seam with
      a `chat/tool-call` hook point and `chat.tool.called` events. Every
      round is its own logged dispatch; after `toolMaxRounds` the model is
      dispatched once with no tools so a turn always ends in text. The chat
      response carries `toolTrace`. `kura-core` stays unwired (recorded: it
      duplicates the dispatcher's retry/cancel/persistence and would have
      meant two live loops). Tests: service-level (round trip, bound, veto,
      no-host, streaming) and assembly-level (`tool_host_tests`).
      *Found and fixed on the way (D10):* there was no live HTTP LLM
      provider at all; `kura-openai-provider` now implements
      `kura_llm::Provider` over `/chat/completions` with tools (complete and
      streamed fragment reassembly), registered when
      `llm.openaiCompatible.baseURL` is set and passes the egress policy.
