...
fn temp_dir(name: &str) -> String {
    let dir =
        std::env::temp_dir().join(format!("kura_store_tenancy_{name}_{}", std::process::id()));
...
fn runs_round_trip_and_tenant_scoping() {
    ...
    store
        .upsert_run_for_tenant_safe(&make_run("run_a", "ten_a"), "ten_a")
        .unwrap();
    store
        .upsert_run_for_tenant_safe(&make_run("run_b", "ten_b"), "ten_b")
        .unwrap();
    ...
    store
        .upsert_run(&make_run("run_unbound", "unused"))
        .unwrap();
...
fn get_run_for_tenant_raw_cross_tenant_is_sentinel() {
    ...
    // Own tenant: found.
    let run = store
        .get_run_for_tenant_raw("run_a", "ten_a")
        .unwrap()
        .expect("owned run");
    ...
    assert!(
        store
            .get_run_for_tenant_raw("nope", "ten_a")
            .unwrap()
            .is_none()
    );
...
fn upsert_run_for_tenant_safe_refuses_cross_tenant_write() {
    ...
    let err = store
        .upsert_run_for_tenant_safe(&hijack, "ten_b")
        .unwrap_err();
    assert!(SQLiteStore::is_cross_tenant_row(&err));
...
fn bind_and_delete_row_for_tenant() {
    ...
    assert_eq!(
        store
            .lookup_row_tenant("runs", "run_id", "run_bind")
            .unwrap(),
        None
    );
    store
        .bind_row_tenant("runs", "run_id", "run_bind", "ten_a")
        .unwrap();
    assert_eq!(
        store
            .lookup_row_tenant("runs", "run_id", "run_bind")
    ...
    let err = store
        .bind_row_tenant("runs", "run_id", "run_bind", "ten_b")
        .unwrap_err();
    assert!(SQLiteStore::is_cross_tenant_row(&err));

    // Delete: own tenant deletes, missing row false, cross-tenant refused.
    store
        .upsert_run_for_tenant_safe(&make_run("run_del", "ten_a"), "ten_a")
        .unwrap();
    assert!(
        store
            .delete_row_for_tenant("runs", "run_id", "run_del", "ten_a")
            .unwrap()
    );
    assert!(
        !store
    ...
    let err = store
        .delete_row_for_tenant("runs", "run_id", "run_bind", "ten_b")
        .unwrap_err();
    assert!(SQLiteStore::is_cross_tenant_row(&err));
}
...
fn steps_and_tool_calls_round_trip_and_scoping() {
    ...
    store
        .upsert_tool_call_for_tenant_safe(&tc, "ten_a")
        .unwrap();
    ...
    let a_tcs = store
        .list_tool_calls_for_tenant_raw("ten_a", "run_st", "step_a")
        .unwrap();
    assert_eq!(a_tcs.len(), 1);
    assert_eq!(a_tcs[0].tool_call_id, "tc_a");
    assert!(
        store
            .list_steps_for_tenant_raw("ten_b", "run_st")
            .unwrap()
            .is_empty()
    );
    assert!(
        store
            .list_tool_calls_for_tenant_raw("ten_b", "run_st", "step_a")
            .unwrap()
            .is_empty()
    );
    ...
    let err = store
        .upsert_step_for_tenant_safe(&hijack, "ten_b")
        .unwrap_err();
    assert!(SQLiteStore::is_cross_tenant_row(&err));
    assert_eq!(
        store.list_steps_for_tenant_raw("ten_a", "run_st").unwrap()[0].title,
        "Do the thing"
    );
}
...
fn sessions_round_trip_and_scoping() {
    ...
    store
        .upsert_session_for_tenant_safe(&make_session("sess_a"), "ten_a")
        .unwrap();
    store
        .upsert_session_for_tenant_safe(&make_session("sess_b"), "ten_b")
        .unwrap();
    ...
    assert_eq!(
        store.list_sessions_for_tenant_raw("ten_b").unwrap()[0].session_id,
        "sess_b"
    );
}
...
fn llm_dispatches_round_trip_and_scoping() {
    ...
    store
        .upsert_llm_dispatch_for_tenant_safe(&make_dispatch("disp_a"), "ten_a")
        .unwrap();
    store
        .upsert_llm_dispatch_for_tenant_safe(&make_dispatch("disp_b"), "ten_b")
        .unwrap();
    ...
    let err = store
        .get_llm_dispatch_for_tenant_raw("disp_a", "ten_b")
        .unwrap_err();
    assert!(SQLiteStore::is_cross_tenant_row(&err));
    assert!(
        store
            .get_llm_dispatch_for_tenant_raw("disp_a", "ten_a")
            .unwrap()
            .is_some()
    );
    assert!(
        store
            .get_llm_dispatch_for_tenant_raw("nope", "ten_a")
            .unwrap()
            .is_none()
    );
    ...
    let err = store
        .upsert_llm_dispatch_for_tenant_safe(&make_dispatch("disp_a"), "ten_b")
        .unwrap_err();
    assert!(SQLiteStore::is_cross_tenant_row(&err));
}
...
fn checkpoints_round_trip_and_scoping() {
    ...
    store.upsert_run_for_tenant_safe(&run, "ten_a").unwrap();
    store
        .save_checkpoint_for_tenant_safe(&make_checkpoint(&run), "ten_a")
        .unwrap();

    let owned = store
        .list_latest_checkpoints_for_tenant_raw("ten_a")
        .unwrap();
    assert_eq!(owned.len(), 1);
    assert_eq!(owned[0].run.run_id, "run_ck");
    assert!(
        store
            .list_latest_checkpoints_for_tenant_raw("ten_b")
            .unwrap()
            .is_empty()
    );
}
...
fn run_exists_for_tenant_does_not_leak() {
    ...
    store
        .upsert_run_for_tenant_safe(&make_run("run_ex", "ten_a"), "ten_a")
        .unwrap();
...
fn events_append_and_list_for_tenant() {
    ...
    let persisted = store
        .append_event_for_tenant_raw(&make_event("evt_a"), "ten_a")
        .unwrap();
    assert_eq!(persisted.tenant_id, "ten_a");
    assert!(persisted.sequence > 0);
    store
        .append_event_for_tenant_raw(&make_event("evt_b"), "ten_b")
        .unwrap();

    let a = store
        .list_events_for_tenant_raw("ten_a", &Filter::default())
        .unwrap();
    assert_eq!(a.len(), 1);
    ...
    let err = store
        .append_event_for_tenant_raw(&make_event("evt_a"), "ten_b")
        .unwrap_err();
    assert!(SQLiteStore::is_cross_tenant_row(&err));
    assert_eq!(
        store
    ...
    assert_eq!(
        store
            .list_events_for_tenant_raw("ten_a", &filter)
            .unwrap()
            .len(),
        1
    );
...
fn approvals_and_decisions_for_tenant() {
    ...
    store
        .bind_row_tenant("approvals", "approval_id", "apr_a", "ten_a")
        .unwrap();
    ...
    store
        .bind_row_tenant("decisions", "decision_id", "dec_a", "ten_a")
        .unwrap();

    assert_eq!(
        store.list_approvals_for_tenant_raw("ten_a").unwrap().len(),
        1
    );
    assert_eq!(
        store.list_decisions_for_tenant_raw("ten_a").unwrap().len(),
        1
    );
    assert!(
        store
            .list_approvals_for_tenant_raw("ten_b")
            .unwrap()
            .is_empty()
    );
    assert!(
        store
            .list_decisions_for_tenant_raw("ten_b")
            .unwrap()
            .is_empty()
    );
}
...
fn default_tenant_resolver_fails_closed_then_resolves() {
    ...
    assert_eq!(
        store.resolve_default_personal_tenant_id().unwrap(),
        "ten_default"
    );
    assert_eq!(
        store.resolve_default_tenant_binding().unwrap(),
        "ten_default"
    );
}
