...
#[cfg(test)]
mod tool_host_tests {
    //! Stage 9.0/9.0b/9.2 against the real assembly: the model asks for a
    //! tool, the assembly's host answers from the real memory / tool planes.

    use std::sync::Arc;

    use futures::future::BoxFuture;
    use kura_chat::{CancellationToken, QueryInput, ToolHost as _};
    use kura_config::{Config, Environment};
    use kura_llm::{
    ...
    fn query(provider: &str, text: &str) -> QueryInput {
        QueryInput {
    ...
    fn memory_lookup_tool_recalls_ready_memory_mid_turn() {
        ...
        let execution = chat
            .query(
                query("openai_compatible", "which package manager?"),
                &CancellationToken::new(),
            )
            .expect("query");
        assert!(execution.exec_error.is_none(), "{:?}", execution.exec_error);
        let result = execution.result;
        assert_eq!(result.tool_trace.len(), 1, "{:?}", result.tool_trace);
        assert!(
            !result.tool_trace[0].is_error,
            "{}",
        ...
        assert!(
            result.tool_trace[0]
                .output
                .contains(&format!("Memory[l1 {}]", atom.asset_id)),
            "hit cites the asset: {}",
            result.tool_trace[0].output
        );
        assert!(result.tool_trace[0].output.contains("pnpm"));
        assert!(!result.tool_trace[0].output.contains("restricted"));
        assert!(result.dispatch.output.starts_with("final: [1] Memory[l1"));
        // The first round offered the memory tool; the assembly built it.
        let offered: Vec<String> = provider.requests.read()[0]
            .tools
            .iter()
            .map(|t| t.name.clone())
            .collect();
        assert!(
            offered.contains(&"memory.lookup".to_string()),
            "{offered:?}"
        );
        assert!(
            !offered.contains(&"web.search".to_string()),
            "no search profile configured"
        );
    }

    /// 9.2 mechanism through the real host: a Ready `web.search` profile is
    ...
    fn a_ready_search_profile_is_offered_and_called_through_the_runtime() {
        ...
        let execution = chat
            .query(
                query("openai_compatible", "search for kura"),
                &CancellationToken::new(),
            )
            .expect("query");
        assert!(execution.exec_error.is_none());
        ...
        assert_eq!(result.tool_trace[0].name, "web.search");
        assert!(
            !result.tool_trace[0].is_error,
            "{}",
            result.tool_trace[0].output
        ...
        assert!(
            result.tool_trace[0]
                .output
                .contains("stub result 1 for kura daemon")
        );
        let offered: Vec<String> = provider.requests.read()[0]
            .tools
            .iter()
            .map(|t| t.name.clone())
            .collect();
        assert!(offered.contains(&"web.search".to_string()));
    }

    /// The Stage 9 verification bar: a tool call meets a quota denial and a
    /// policy denial, and both reach the model as tool errors — never as a
    /// silent skip, never as a call that ran anyway.
    #[test]
    fn a_tool_call_meets_quota_and_policy_denials_as_tool_errors() {
        struct DenyAll;
        ...
        let host = crate::tool_host::AppToolHost::new(late);
        let ctx = kura_chat::ToolContext::default();
        let call = kura_llm::ToolCall {
            call_id: "c".into(),
            name: "web.search".into(),
            arguments: "{\"query\":\"x\"}".into(),
        };
        // Quota denial: the runtime refuses before the provider runs.
        let outcome = host.call(&ctx, &call);
        assert!(outcome.is_error);
        assert!(outcome.content.contains("quota denied: quota_exhausted"), "{}", outcome.content);

        // Policy denial: a `chat/tool-call` hook vetoes; the host is never asked.
        struct Veto;
        impl kura_plugin::Hook for Veto {
            fn handle(&self, _p: &mut serde_json::Value) -> kura_plugin::HookOutcome {
                kura_plugin::HookOutcome::Halt("no search in this workspace".to_string())
            }
        }
        let hooks = app.state.hooks.clone().expect("hooks");
        hooks.register(kura_plugin::points::CHAT_TOOL_CALL, "policy-test", Arc::new(Veto));
        let mut payload = serde_json::json!({ "name": "web.search", "arguments": "{}" });
        let run = hooks.run(kura_plugin::points::CHAT_TOOL_CALL, &mut payload);
        let (plugin, reason) = run.halted.expect("vetoed");
        assert_eq!(plugin, "policy-test");
        assert_eq!(reason, "no search in this workspace");
    }

    /// An `mcp_backed` profile whose server is unknown is not offered (the
    /// model must not be shown a tool that cannot be called) and a direct
    /// call is refused with a reason.
    #[test]
    fn an_mcp_backed_profile_without_a_live_server_is_not_offered() {
        ...
        assert!(outcome.is_error);
        assert!(
            outcome.content.contains("mcp server not found"),
            "{}",
            outcome.content
        );
    }
