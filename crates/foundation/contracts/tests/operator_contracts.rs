//! Ported from daemon/internal/contracts/operator_contracts_test.go (wave 8 contract parity).
//!
...
#[test]
fn test_operator_schemas_accept_canonical_fixtures() {
    let validator = Validator::new(schema_root_dir());
    validate_fixtures(
        &validator,
        &[common::data::operator_contract_fixtures()].concat(),
    );
}
