//! Ported from daemon/internal/contracts/integration_diagnostics_test.go (wave 8 contract parity).
//!
...
#[test]
fn test_integration_diagnostic_schemas_accept_canonical_fixtures() {
    let validator = Validator::new(schema_root_dir());
    validate_fixtures(
        &validator,
        &[common::data::integration_diagnostic_contract_fixtures()].concat(),
    );
}
