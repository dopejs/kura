//! Ported from daemon/internal/contracts/billing_contracts_test.go (wave 8 contract parity).
//!
...
#[test]
fn test_billing_schemas_accept_canonical_fixtures() {
    let validator = Validator::new(schema_root_dir());
    validate_fixtures(
        &validator,
        &[common::data::billing_contract_fixtures()].concat(),
    );
}
