//! Ported from daemon/internal/contracts/matrix_channel_connector_test.go (wave 8 contract parity).
//!
...
#[test]
fn test_matrix_connector_schemas_accept_canonical_fixtures() {
    let validator = Validator::new(schema_root_dir());
    validate_fixtures(
        &validator,
        &[common::data::matrix_connector_contract_fixtures()].concat(),
    );
}
